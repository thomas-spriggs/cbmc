...
#include <solvers/smt2_incremental/expressions.h>
...
exprt
smt2_incremental_decision_proceduret::substitute_defined_padding(
  exprt root_expr)
{
  root_expr.visit_pre([&](exprt &node) {
      if(const auto pad = expr_try_dynamic_cast<nondet_padding_exprt>(node))
      {
        ...
        const auto term =
          smt_identifier_termt{instance, convert_type_to_smt_sort(node.type())};
...
smt2_incremental_decision_proceduret::convert_expr_to_smt(const exprt &expr)
{
  ...
  const exprt substituted = substitute_defined_padding(substitute_identifiers(
    expr, expression_identifiers));
