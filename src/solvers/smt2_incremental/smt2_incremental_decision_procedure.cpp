  ...
  const exprt &expr)
{
  expr.visit_pre([&](const exprt &expr_node) {
    ...
    if(const auto with_expr = expr_try_dynamic_cast<with_exprt>(expr_node))
    {
      ...
      const auto index_definition = smt_define_function_commandt
        {index_identifier, {}, index_term};
      ...
      solver_process->send(smt_define_function_commandt
                           {index_identifier, {}, index_term});
    }
  });
}
...
exprt smt2_incremental_decision_proceduret::get(const exprt &expr) const
{
  ...
  auto descriptor = [&]() -> optionalt<smt_termt>
  {
    ...
    {
      const auto array =
        get_identifier(index_expr->array(), expression_handle_identifiers, expression_identifiers);
      const auto index =
        get_identifier(index_expr->index(), expression_handle_identifiers, expression_identifiers);
      if(!array || ! index)
        return {};
      return smt_array_theoryt::select(*array, *index);
    }
    return get_identifier(expr, expression_handle_identifiers, expression_identifiers);
  }();
  if(!descriptor)
  {
    ...
    else
    {
      ...
      INVARIANT(
        symbol_expr, "Unhandled expressions are expected to be symbols - " + expr.pretty(0,0));
