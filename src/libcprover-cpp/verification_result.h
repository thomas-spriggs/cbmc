...
class property_infot;
using propertiest = std::map<irep_idt, property_infot>;
enum class resultt;

// The enumerations here mirror the ones in properties.h.
// The reason we do so is that we want to expose the same information to users
// of the API, without including (and therefore, exposing) CBMC internal headers.
